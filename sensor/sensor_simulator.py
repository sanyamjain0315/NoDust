...
# Changed: Now takes total number of sites and generates IDs automatically
...
POLLUTANTS = {
    "PM2.5": {"base_min": 5,  "base_max": 25,  "spike_min": 80,  "spike_max": 150},
    "NO2":   {"base_min": 10, "base_max": 40,  "spike_min": 180, "spike_max": 350},
    ...
    for site in SITE_IDS:
        ...
        for pollutant, config in POLLUTANTS.items():
            SITE_STATES[site][pollutant] = {
                "state": "NORMAL",
                "current_val": random.uniform(config["base_min"], config["base_max"]),
                "steps_left": 0,
                "target_val": 0.0
            }
...
def update_site_concentration(site, pollutant):
    ...
    if state == "NORMAL":
        ...
        if random.random() < 0.01:
            ...
            state_info["target_val"] = random.uniform(config["spike_min"], config["spike_max"])
        ...
        if state_info["steps_left"] <= 0:
            ...
            state_info["target_val"] = random.uniform(config["base_min"], config["base_max"])
